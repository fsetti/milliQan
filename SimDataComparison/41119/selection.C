...
void selection( TChain *chain, TString output_filename, bool Data = true ) 
{
  ...
  float NchanHits[nChannels];
  float nNchanHits[nChannels];
  ...
  TH1::SetDefaultSumw2();
  TH1D *h_nPE, *h_nNnPE, *h_NnPE, *h_chN, *h_chnN;
  ...
  h_chN = new TH1D("lowNPE_chan_N","chan, nPE < 20 ", 32,0,32);
  h_chnN = new TH1D("lowNPE_chan_nN","chan, nPE < 20 ", 32,0,32);
