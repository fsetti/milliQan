...
//		h_nNnPE->Fill ( chan_trueNPE[(int)nonNeighPulses.at(i).at(0)], weight );
	...
	for ( unsigned int i=0; i<neighPulses.size(); i++){
		if ( neighPulses.at(i).at(3) < 10 ) outfile << fileID << " " << orig_evt << " " << (int)neighPulses.at(i).at(0) << endl;
//		cout << "reco nPE: " << (int)neighPulses.at(i).at(3) << ", MCtruth: " << chan_trueNPE[(int)neighPulses.at(i).at(0)] << ", channel: " << (int)neighPulses.at(i).at(0) << endl ;
