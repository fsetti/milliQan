...
#include "/home/users/fsetti/milliQan/Headers/CmuonAnalysis.h"
...
vector<vector<float>> MuonPulses ( vector<int> *pulseChan, vector<float> *pulseHeight, vector<float> *pulseTime, vector<float> *pulsenPE, vector<float> *pulseTimeCal, vector<int> TrigChannels ){
...
// update selection for comparison of non-neighbouring pulses with MC (signal injected?)
vector<vector<float>> CosmicSelection_v2 ( vector<int> *pulseChan, vector<float> *pulseHeight, vector<float> *pulseTime, vector<float> *pulsenPE, vector<float> *pulseTimeCal, vector<int> TrigChannels ){
        vector<vector<float>> preSelPulses;
        for (unsigned int j=0; j<pulseChan->size(); j++){
                for (unsigned int k=0; k<TrigChannels.size(); k++){
                        if( pulseChan->at(j)   == TrigChannels.at(k)
...
// check for nPE above muon threshold
        for (unsigned int j=0; j<preSelPulses.size(); j++){
		if ( preSelPulses.at(j).at(3) < nPEminVec.at((int)preSelPulses.at(j).at(0)) ){
...
vector<vector<float>> SameLayerActivity_v2 ( vector<int> *pulseChan, vector<float> *pulseHeight, vector<float> *pulseTime, vector<float> *pulsenPE, vector<float> *pulseTimeCal ){

        vector<vector<float>> pulses;
        for (unsigned int j=0; j<pulseChan->size(); j++){
                for (unsigned int k=0; k<trigBarsPanelsSlabs.size(); k++){
                        if( pulseChan->at(j)   == trigBarsPanelsSlabs.at(k)
                         && pulsenPE->at(j)  >  nPEmin
                         && pulseTime->at(j)  <  tMax
                         && pulseTime->at(j) >  tMin
                                ){
                                pulses.push_back({(float)pulseChan->at(j), pulseHeight->at(j), pulseTime->at(j), pulsenPE->at(j), pulseTimeCal->at(j)});
				break;
                        }
                }
        }

        std::sort( pulses.begin(), pulses.end(), sort_wrt_calTime );
        for (unsigned int j=0; j<pulses.size(); j++){
                for (unsigned int k=j+1; k<pulses.size(); k++){
                        if ( pulses.at(j)[0] == pulses.at(k)[0] ){
                                pulses.at(j).at(3)+= pulses.at(k).at(3);
                                pulses.erase( pulses.begin() + k );
                                k -=1;
                        }
                }
        }
...
vector<vector<float>> CosmicsNeighBarSelection_diag( vector<int> *pulseChan, vector<float> *pulseHeight, vector<float> *pulseTime, vector<float> *pulsenPE, vector<float> *pulseTimeCal, vector<int> TrigChannels ){
        vector<vector<float>> preSelPulses;
        for (unsigned int j=0; j<pulseChan->size(); j++){
                for (unsigned int k=0; k<TrigChannels.size(); k++){
                        if( pulseChan->at(j)   == TrigChannels.at(k)
